# 🛠️ Mincheol An
> **Aspiring Robotics & Embedded Systems Engineer**
> *Driven by Passion and Precision.*

---

### 🎓 About
- **Sangmyung Univ.** Computer Science (Expected Feb 2027)
- **Focus:** Embedded Systems, Robotics Control, Linux Kernel

### 💻 Technical Stack
- **Languages:** `C`, `Python`, `Assembly`
- **Hardware:** `Jetson Nano`, `STM32`, `ESP32`, `Raspberry Pi`
- **Tools:** `Vim`, `Linux(Ubuntu)`, `Git`

---
[![](https://github-readme-stats.vercel.app/api?username=anmincheol-71&show_icons=true&theme=blood&hide_border=true&count_private=true)](https://github.com/anmincheol-71)
https://mincheol701.tistory.com/
